...
export function ViewControls({
  ...
  const isAttackerPerspective = injectionPerspective === "attacker";

  return (
    <div
      className="view-controls"
      data-view-mode={viewMode}
      data-gamification={isDemo && showGamification ? "true" : undefined}
    >
      <div className="view-controls-inner">
        ...
        {isDemo ? (
          <div className="view-controls-group" aria-label={copy.viewControls.intelOptionsAria}>
            <button
              ...
              aria-pressed={!isAttackerPerspective}
            >
              <span className="sm:hidden">
                {isAttackerPerspective ? copy.viewControls.attackShort : copy.viewControls.defenseShort}
              </span>
              <span className="hidden sm:inline">
                {isAttackerPerspective
                  ? copy.viewControls.perspectiveAttacker
                  : copy.viewControls.perspectiveDefender}
              </span>
            </button>
            <button
              type="button"
            ...
            >
              <span className="sm:hidden">
                {showGamification ? copy.viewControls.xpOnShort : copy.viewControls.xpOffShort}
              </span>
              <span className="hidden sm:inline">
                {showGamification ? copy.viewControls.xpOn : copy.viewControls.xpOff}
              </span>
            </button>
          </div>
        ) : null}
        ...
        <div className="view-controls-group" aria-label={copy.viewControls.localeSwitchAria}>
          <button
            ...
            className={`view-switch-btn ${locale === "de" ? "active" : ""}`}
          >
            {copy.viewControls.localeDe}
          </button>
          <button
            type="button"
            onClick={() => setLocale("en")}
            className={`view-switch-btn ${locale === "en" ? "active" : ""}`}
          >
            {copy.viewControls.localeEn}
          </button>
        </div>
      </div>
    </div>
