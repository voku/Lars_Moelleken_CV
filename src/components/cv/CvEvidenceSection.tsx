...
export type CvEvidenceSectionKey = "technologies" | "education" | "projects" | "personal";

const ALL_SECTIONS: readonly CvEvidenceSectionKey[] = ["technologies", "education", "projects", "personal"];

interface CvEvidenceSectionProps {
  readonly theme?: ComponentTheme;
  readonly sections?: readonly CvEvidenceSectionKey[];
}

export function CvEvidenceSection({ theme = "standard", sections = ALL_SECTIONS }: CvEvidenceSectionProps) {
  ...
  const showTechnologies = sections.includes("technologies");
  ...
  return (
    <section aria-label={locale === "de" ? "Weitere CV-Evidenz" : "Additional CV evidence"} className="space-y-5">
      {showTechnologies || showEducation ? (
